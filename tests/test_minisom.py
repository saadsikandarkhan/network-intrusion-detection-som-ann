"""Unit tests for MiniSom (migrated from inline tests in original minisom.py)."""

import numpy as np
from numpy.testing import assert_almost_equal, assert_array_almost_equal, assert_array_equal

from src.som_anomaly_detector.minisom import MiniSom, fast_norm
...
class TestMiniSom:
    def setup_method(self):
        ...
        for i in range(5):
            for j in range(5):
                assert_almost_equal(1.0, np.linalg.norm(self.som.weights[i, j]))
        self.som.weights = np.zeros((5, 5))
    ...
    def test_random_weights_init(self):
        ...
        for w in som.weights:
            assert_array_equal(w[0], np.array([1.0, 0.0]))
